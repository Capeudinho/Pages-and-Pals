...
public interface BookshelfRepository extends JpaRepository<Bookshelf, Long> {
	...
	@Query("SELECT bookshelf FROM Bookshelf bookshelf WHERE (bookshelf.owner.privacy = true OR bookshelf.owner.id = :ownerId) AND (EXISTS (SELECT bookApiId FROM bookshelf.bookApiIds bookApiId WHERE bookApiId IN :bookApiIds) OR ((:bookshelfName IS NOT null OR :ownerName IS NOT null) AND (:bookshelfName IS null OR LOWER(bookshelf.name) LIKE LOWER(CONCAT('%', :bookshelfName, '%'))) AND (:ownerName IS null OR LOWER(bookshelf.owner.name) LIKE LOWER(CONCAT('%', :ownerName, '%'))))) ORDER BY bookshelf.creationDate DESC")
