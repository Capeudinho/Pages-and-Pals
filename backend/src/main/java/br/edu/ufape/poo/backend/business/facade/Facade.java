...
public class Facade
{
	...
	public List<Map<String, Object>> bookshelfFindOwnByOwnerIdPaginate(Long ownerId, int offset, int limit, String email, String password) throws Exception
	{
		...
		List<Map<String, Object>> bookshelfCards = bookshelfFindByOwnerIdPaginateUtility(requestingAccount, offset, limit, true);
	...
	{
		Account requestingAccount = accountService.authenticate(email, password);
		List<Map<String, Object>> bookshelfSelects = bookshelfFindSelectByOwnerIdUtility(requestingAccount, apiId, true);
	...
	private List<Map<String, Object>> bookshelfFindByOwnerIdPaginateUtility(Account account, int offset, int limit, boolean complete) throws Exception
	{
		int bookshelfCount = bookshelfService.countByOwnerId(account.getId());
		if (offset < 0)
		{
		...
		for (int bookshelfIndex = 0; bookshelfIndex < bookshelves.size(); bookshelfIndex++)
		{
			...
			{
				List<String> covers = new ArrayList<String>();
				for (int coverIndex = 0; coverIndex < bookshelf.getBookApiIds().size() && coverIndex < 3; coverIndex++)
				{
	...
	private List<Map<String, Object>> bookshelfFindBooksByIdPaginateUtility(Bookshelf bookshelf, int offset, int limit) throws Exception
	{
		if (offset < 0)
		{
			offset = 0;
	...
	private List<Map<String, Object>> bookshelfFindSelectByOwnerIdUtility(Account account, String apiId, boolean complete) throws Exception
	{
