...
public class ReviewService implements ReviewServiceInterface {
	...
	public Review create(Review review) throws Exception {
		Review existingReview = reviewRepository.findByOwnerIdAndBookApiId(review.getOwner().getId(),
				review.getBookApiId());
		...
		if (review.getBookScore() != null
				&& (review.getBookScore() <= 0 || review.getBookScore() > 5 || review.getBookScore() % 0.5 != 0)) {
			throw new InvalidBookScoreException();
		}
	...
	public List<Review> findByBookApiIdAndPublicOrOwnerIdPaginate(String bookApiId, long ownerId, int offset,
			int limit) {
		Pageable pageable = new OffsetPageRequest(offset, limit);
		List<Review> reviews = reviewRepository.findByBookApiIdAndPrivacyOrOwnerIdOrderByCreationDateDesc(bookApiId,
				ownerId, true, pageable);
		return reviews;
	}
	...
	public List<Review> findByBookApiIdAndPublicPaginate(String bookApiId, int offset, int limit) {
		Pageable pageable = new OffsetPageRequest(offset, limit);
		List<Review> reviews = reviewRepository.findByBookApiIdAndPrivacyOrderByCreationDateDesc(bookApiId, true,
				pageable);
		return reviews;
	}
